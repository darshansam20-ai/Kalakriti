...
export const Home: React.FC = () => {
  ...
  const [heroContent, setHeroContent] = useState({
    ...
    subtitle: 'Authentic Indian artistry woven into timeless handcrafted jewelry.'
  });
  ...
  const customMadeProducts = products.filter(p => p.categories?.includes('Custom Made')).slice(0, 4);
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <section className="py-16 bg-bg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10">
          <div className="flex justify-between items-end mb-8">
            <div>
              <h2 className="font-serif text-[24px] text-ink font-bold">Custom Made</h2>
            </div>
            <Link to="/shop?category=custom-made" className="hidden sm:block text-text-light hover:text-ink font-medium text-[13px] uppercase tracking-wide">
              View All
            </Link>
          </div>

          <div className="grid grid-cols-2 lg:grid-cols-3 gap-[20px]">
            {customMadeProducts.slice(0, 3).map((product, index) => (
              <motion.div 
                key={product.id}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
          ...
          <div className="mt-8 text-center sm:hidden">
            <Link to="/shop?category=custom-made" className="inline-block border border-gold text-ink font-medium px-6 py-3 rounded-[30px] text-[13px] uppercase tracking-wide">
              View All Products
            </Link>
          </div>
        </div>
